...
{ int i, j, unlock_failure=1, end=start+len-1;
...
int dbase_lock(struct dbase_header *th, long firstrec, long reccount, int retries);
...
long dbase_query(struct dbase_header *th, long *recno, char **fields, int *mode, int indexval)
{ int i,k,found,j,offs,offsets[MAXFIELDS],buffers,rbuflen,rlength,t, b;
  char *comparestrings[MAXFIELDS],*p,*q,*pbuffer;
  long record, count, returnvalue;
...
int dbase_read(dbase_header *th, long recno, char** fields)
...
int dbase_write(dbase_header *th, long recno, char** fields, int mark)
...
int dbase_trunc(dbase_header *th, long reccount)
{ long file_end;
...
{ int i, j, k, m, hashfield=-1, offset, len;
  int t = GetTickCount();
  char *field;
  ...
  if (totalrec > 100) i = (int)(500000.0*(float)MBextra*(float)th->reccount/(float)totalrec);
  else                i = (int)(500000.0*(float)MBextra/4.0);
  if (i<10000)        i = 10000;
...
extern	char	ConfigFile[];
  ...
  { HINSTANCE hInst;
    ...
    { MessageBox(NULL, "*** Fatal error: could not load libmysql.dll to access mysql", "dgate.exe", 0);
      exit(999);
    }
  }
#endif
...
Database :: Database()
	...
	GetPrivateProfileString ( RootSC, "Postgres", "0", (char*) Temp, 128, ConfigFile);
	Postgres = atoi(Temp);
	...
	GetPrivateProfileString ( RootSC, "Mysql", "0", (char*) Temp, 128, ConfigFile);
	Mysql = atoi(Temp);
	...
	GetPrivateProfileString ( RootSC, "SqLite", "0", (char*) Temp, 128, ConfigFile);
	SqLite = atoi(Temp);
...
BOOL	Database :: Open (
	...
	char	*User,
	char	*Password,
	char 	*ServerName)	// used in Sybase context, ignored here
	{
	if ( Connected )
		Close();
	...
	if (SqLite)
	{
	 	if (sqlite3_open(DataSource, &sqlitedata))
			{
	...
	// built-in dbase support (when datasource is a directory, must end with \)
	if (strchr(DataSource, PATHSEPCHAR))
	{ 
	  ...
	  OdbcDoubleBackSlashToDB = atoi(Temp);
  	  ...
  	  GetPrivateProfileString ( RootSC, "PackDBF", "0",
		(char*) Temp, 128, ConfigFile);
	  PackDBF = atoi(Temp);
	  ...
	  if (NeedPack==2) PackDBF = FALSE;     /* 20 MB index, no pack - used during full regen (please clear db first) */
  	  ...
  	  GetPrivateProfileString ( RootSC, "IndexDBF", "1",
		(char*) Temp, 128, ConfigFile);
	  IndexDBF = atoi(Temp);
	  if (NeedPack>1 && IndexDBF<20) IndexDBF=20;	// allocate at least 20 MB during regen - (for 2.5*10^6 images)
  	  ...
  	  GetPrivateProfileString ( RootSC, "LongQueryDBF", "1000",
		(char*) Temp, 128, ConfigFile);
	  LongQueryDBF = atoi(Temp);

          strcpy(dir, DataSource);

	  strcpy(TempPath, DataSource);
	  strcat(TempPath, "*.dbf");
	...
	if (Postgres)
	{
		...
		conn = PQsetdbLogin(DataHost,NULL,NULL,NULL,DataSource,User,Password);
		if (PQstatus(conn) == CONNECTION_BAD)
		{
			SystemDebug.printf("%s\n",PQerrorMessage(conn));
			PQfinish(conn);
			conn = NULL;
			return (FALSE);
		}
	...
	{	
...
#ifndef DARWIN
		if (!((mydata = mysql_init((MYSQL*)NULL)) && mysql_real_connect(mydata,DataHost,User,Password,NULL,MYSQL_PORT,NULL,0)))
		{
		...
		if (mysql_select_db( mydata, DataSource)<0)
		{
	...
	RetCode =  SQLConnect(hDbc, (BYTE*)DataSource, SQL_NTS,
						(BYTE*)User, SQL_NTS, (BYTE*)Password, SQL_NTS);
...
/*OperatorConsole.printf("sqlite_exec: %-.60s\n", StatementText);*/
...
/*OperatorConsole.printf("returns: %d %s\n", rc, err);*/
...
BOOL	Database :: Query (	char	*Table,
				char	*Columns,
				char	*Where,
				char	*Order )
	{ if (DebugLevel>=4)
...
BOOL	Database :: InternalQuery (	char	*Table,
				char	*Columns,
				char	*Where,
				char	*Order,
				BOOL	recurse )
	...
	{ char *tmp = (char *)malloc(MAXQUERYLENGTH); 	// need a lot of space!
	  ...
	  strcpy(tmp, Table);
		...
		if (p) r = p+1; else r = NULL;
                ...
                q = strrchr(p, '\'');			// value must be enclosed by '
                if (!q) continue;
                *q=0;
                q = strchr(p, '\'');
                if (!q) continue;
...
BOOL	Database :: AddRecord (	char	*Table,
				char	*Columns,
				char	*Values )
	{
...
BOOL	Database :: DeleteRecord (	char	*Table,
					char	*Where )
	{
...
BOOL	Database :: BindField (	UWORD	Column,
	...
	{ 
		...
		{ 
		case SQL_C_CHAR:
				*LengthNeeded = PQfmod(res,Column-1) -4 ;
				break;
		case SQL_C_ULONG: 
				*LengthNeeded = 4; 
				break;
...
//OperatorConsole.printf("Hashing found record in db %s: %d\n", thq->filename, queryrecno);
	    ...
	    if (queryrecno)
          ...
          { if (!strings[i]) break;
            ...
            { case SQL_C_CHAR:  strcpy((char *)(vPtrs[n]), strings[i]); *LengthNeededs[n] = strlen(strings[i]); break;
              case SQL_C_ULONG: *(unsigned long *)(vPtrs[n]) = atol(strings[i]); *LengthNeededs[n]=4; break;
	...
	if (Postgres)
	{
		...
		for(i=0;i<ncolumns;i++)
		{
			...
			{
			case SQL_C_CHAR:
				strcpy((char *)(vPtrs[i]),PQgetvalue(res,current_tuple,i));
				*LengthNeededs[i] = PQgetlength(res,current_tuple,i);
				break;
			case SQL_C_ULONG:
				*(unsigned long *)(vPtrs[i]) = atol(PQgetvalue(res,current_tuple,i));
				*LengthNeededs[i] = 4;
				break;
			default:
				break;
		...
		if(row[i]) //Steini 20050507
			...
			case SQL_C_CHAR:
				...
				*LengthNeededs[i] = lengths[i]; // strlen(row[i]);
				break;
			case SQL_C_ULONG:
				*(unsigned long *)(vPtrs[i]) = atol(row[i]);
				*LengthNeededs[i] = 4;
				break;
			default:
				break;
	...
	if (SqLite)
	{
		...
		{	switch (CTypeCodes[i])
			...
			case SQL_C_CHAR:
				  ...
				  strcpy((char *)(vPtrs[i]), resultp[(current_tuple+1)*ncolumns+i]);
				else
				  ((char *)(vPtrs[i]))[0] = 0;
			...
			case SQL_C_ULONG:
			        if (resultp[(current_tuple+1)*ncolumns+i])
				  *(unsigned long *)(vPtrs[i]) = atol(resultp[(current_tuple+1)*ncolumns+i]);
				else
				  *(unsigned long *)(vPtrs[i]) = 0;
				*LengthNeededs[i] = 4;
				break;
...
BOOL	Database :: CreateDatabase (
	...
	NewStatement();
	sprintf (SQLStatement, "exec sp_addlogin '%s','%s','%s'", User, Password, Name);
	i = SQLExecDirectWithRetry ( hStmt, (unsigned char *) SQLStatement, SQL_NTS );
        if (i != SQL_SUCCESS_WITH_INFO && i != SQL_SUCCESS)
		{ 
		SystemDebug.printf ( "***Failed SQLExecDirect : %.1000s\n", SQLStatement);
