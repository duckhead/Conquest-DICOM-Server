...
BOOL recompress(DICOMDataObject **pDDO, char *Compression, char *Filename="", BOOL StripGroup2=FALSE);
...
BOOL	SaveToDataBase (Database &DB, DICOMDataObject	*, char	*filename, char *Device, BOOL JustAdd);
BOOL	CheckOrMakeTable(Database	&);
BOOL	DropTables();
BOOL	MakeSafeString ( VR *, char	*);
...
char	*UniqueKey(DBENTRY*);
char	*UniqueLink(DBENTRY*);
BOOL	UpdateAccessTimes(Database &, char *);
BOOL	ChangeUID(char *OldUID, char *Type, char *NewUID);
BOOL	MergeUIDs(char *OldUID[], int n, char *Type, char *NewUID);
UINT32	SQLRealSize(char	*Str, SDWORD	Max);
BOOL	Regen();
BOOL	Regen(char*, int IsCacheOrJukeBox, char *SubDir = NULL);
...
BOOL	GetPhysicalDevice(char	*Dev, char	*Name);
