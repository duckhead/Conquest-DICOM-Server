...
static
int
GetFileNameSyntax(int *maxfilenamelength)
	{
	...
	if (FileNameSyntax<0)
		...
		GetPrivateProfileString ( RootSC, "FileNameSyntax", "0",
			(char*) FileNameSyntaxString, 256, ConfigFile);
		FileNameSyntax = atoi(FileNameSyntaxString);
		if (strchr(FileNameSyntaxString, '%')) FileNameSyntax=999;	// configured

		GetPrivateProfileString ( RootSC, "MaxFileNameLength", "255",
...
GenerateFileName(
	...
	VR	*TempVR;
	VR	*StudyDate;
	int	iSeries, iImage, Index, maxfilenamelength;
	...
	switch (iFileNameSyntax)
		...
		strcpy(Root, CName);
	        strcat(Root, "_");
	  	strcat(Root, CID);
		sprintf(filename, "%s%s%04d_%06d_%u%04x.v2", Root, PATHSEPSTR, iSeries, iImage, time(NULL), (gl_iFileCounter++) & 0xffff);
		break;
		...
		strcpy(Root, CName);
	        strcat(Root, "_");
	  	strcat(Root, CID);
		sprintf(filename, "%s%s%s_%04d_%06d_%u%04x.v2", Root, PATHSEPSTR, CSeriesUID, iSeries, iImage, time(NULL), (gl_iFileCounter++) & 0xffff);
		break;
		...
		case 4:
	  	CID [16] = '\0';
		strcpy(Root, CID);
		sprintf(filename, "%s%s%s_%04d_%06d_%u%04x.dcm", Root, PATHSEPSTR, CSeriesUID, iSeries, iImage, time(NULL), (gl_iFileCounter++) & 0xffff);
		break;
		...
		case 999:	// configured filename: any string with %, e.g., %studydate[0,3]\%name_%id\%sopuid.v2
		    ...
		    else if (memcmp(p, "time"     , 4)==0) i+=4, sprintf(filename+strlen(filename), "%u", time(NULL));
		...
		strcpy(Root, CName);
	        strcat(Root, "_");
	  	strcat(Root, CID);
		sprintf(filename, "%s%s%d_%d_%u.v2", Root, PATHSEPSTR, iSeries, iImage, time(NULL));
		break;
		}
