...
{ int           filehandle;             /* info calculated at dbopen */
  ...
  long          lockcount;              /* lockcount = 0 for unlocked */
  ...
  long          bufrecord;              /* record number in buffer */
  char          *rawbuffer;
  long          rawrecord;              /* idem in raw buffer */
  ...
  short int     rlength;
  char          junk2[4];
  long          updatecount;            /* multiuser ext. moved on 01-09-95 */
  char          junk3[8];               /* should contain zeros */
  long          oldupdatecount;         /* old MBI30 multiuser extension */
...
enum DBTYPE {DT_ODBC, DT_DBASEIII, DT_POSTGRES, DT_MYSQL, DT_SQLITE, DT_DBASEIIINOINDEX};
...
//	void *sqlitedata;
	...
	public:
		...
		virtual	BOOL	Open (	char *Database, char	*User, char	*Password,
								char *Server = NULL);
...
#endif
		virtual	BOOL	Query (	char	*Table,
						char	*Columns,
						char	*Where = NULL ,
						char	*Order = NULL );
		virtual	BOOL	InternalQuery (	char	*Table,
						char	*Columns,
						char	*Where = NULL ,
						char	*Order = NULL ,
						BOOL	recurse = FALSE);
		virtual	BOOL	QueryDistinct (	char	*Table,
						...
						char	*Where = NULL ,
						char	*Order = NULL );
		virtual	BOOL	DeleteRecord ( char	*Table,
						 char	*Where );
		virtual	BOOL	AddRecord (	char	*Table,
						char	*Columns,
						char	*Values );
		virtual	BOOL	DeleteTable ( char	*Table );
		virtual	BOOL	CreateTable ( char	*TableName, char	*Columns );
		virtual	BOOL	CreateIndex ( char	*Table,	char	*Index,	char	*Column );
		virtual	BOOL	CreateUniqueIndex ( char	*Table,	char	*Index,	char	*Column );
		virtual	BOOL	UpdateRecords ( char	*TableName,
						char	*Updates,
						char	*Where);
		virtual	BOOL	ExtractRecords( char	*TableName,
						char	*Where,
						char	*Filename);
